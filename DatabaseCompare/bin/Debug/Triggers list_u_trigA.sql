...
origin: list_u_trig'
	...
	if exists (select *  from inserted
		...
		begin
		raiserror 44445 n' 
stavljanjedokumenata u nadzornu knjigu
...
origin: list_u_trig'
	...
	begin
		...
		begin
			...
			from   inserted 
				inner join dbo.dokstart on inserted.id = dbo.dokstart.listid
				inner join dbo.dokvrsta on dbo.dokstart.dokvrsta = dbo.dokvrsta.sifra 
				inner join dbo.dokvrstastatus on dbo.dokstart.status = dbo.dokvrstastatus.sifra 
							and dbo.dokstart.dokvrsta = dbo.dokvrstastatus.dokvrsta
				inner join dbo.dokvrstastatusakcija on dbo.dokvrstastatus.akcija = dbo.dokvrstastatusakcija.sifra
				inner join dbo.dokvk on dbo.dokvrstastatusakcija.autododvk = dbo.dokvk.sifra
		...
		begin
			...
			from   inserted 
				...
				inner join dbo.dokvk dsvk on dbo.dokvk.autopredvk = dsvk.sifra 
				inner join dbo.dokvrsta dokvrsta_1 on dsvk.dokvrsta = dokvrsta_1.sifra
				left outer join dbo.dokstart dokstart_1 on inserted.id = dokstart_1.listid 
			...
			from   inserted 
				...
				inner join dbo.dokum on dbo.dokstart.id = dbo.dokum.dokumid
				inner join dbo.dokvrsta on dbo.dokstart.dokvrsta = dbo.dokvrsta.sifra 
				inner join dbo.dokvk dsvk on dbo.dokstart.vk = dsvk.sifra 
				inner join dbo.dokstart dokstart_1 on inserted.id = dokstart_1.listid 
							and dsvk.autopredvk = dokstart_1.vk 
				inner join dbo.dokvk on dokstart_1.vk = dbo.dokvk.sifra 
				inner join dbo.dokvrsta dokvrsta_1 on dokstart_1.dokvrsta = dokvrsta_1.sifra
				left outer join dbo.dokum dokum_1 on dokstart_1.id = dokum_1.dokumid
	...
	origin: list_u_trig'
		...
		if exists (select *  from inserted
					...
					inner join dbo.dokvrsta with(nolock) on dbo.dokstart.dokvrsta = dbo.dokvrsta.sifra
					inner join dbo.dokvk with(nolock) on dbo.dokstart.vk = dbo.dokvk.sifra
					left outer join dbo.dokvrstaverif with(nolock) on dbo.dokstart.dokvrsta = dbo.dokvrstaverif.dokvrsta 
				where dokvrsta.limitctrl = 1 or dokvrstaverif.[auto] = 1 or dokvk.autoverif > 0)
		begin
			...
			select dokvrstaverif.verifvrsta, dokstart.id 
			from inserted 
				inner join dbo.dokstart with(nolock)on inserted.id = dbo.dokstart.listid 
				inner join dbo.dokvrsta with(nolock)on dbo.dokstart.dokvrsta = dbo.dokvrsta.sifra
				inner join dbo.dokvrstaverif with(nolock)on dbo.dokvrsta.sifra = dbo.dokvrstaverif.dokvrsta 
									and dbo.dokvrstaverif.verifvrsta = 55
				inner join dbo.dokum with(nolock)on dbo.dokstart.id = dbo.dokum.dokumid
				inner join dbo.tbsaldopartner() tbsaldopartner on dbo.dokum.analitika1id = tbsaldopartner.analitikaid 
				left outer join dbo.verifikacija with(nolock)on dbo.dokstart.id = dbo.verifikacija.dokumid
									and dbo.verifikacija.verifvrsta = dbo.dokvrstaverif.verifvrsta
			where dokvrstaverif.verifvrsta = 55 and dokstart.id > 0
		...
		if exists (select * from inserted
				...
				where dv.zadrazd = 1 and dv.zt = 1 and dv.zaliha = 1)
		begin
			update dbo.dokum
			...
			from (select distinct dokstart_1.id
				from inserted
				 ...
				 where dbo.dokstart.azuriranje = 1 and dbo.dokstart.kontrola = 1
					...
					and dokstart_1.azuriranje = 0 and dokstart_1.kontrola = 0
					and dv.zadrazd = 1 and dv.zaliha = 1 and dv.zt = 1) dr
		...
		begin
			...
			from   inserted 
				...
				inner join dbo.dokvk on dbo.dokstart.vk = dbo.dokvk.sifra
				inner join dbo.dokvk dsvk on dbo.dokvk.autopopratvk = dsvk.sifra 
			...
			from inserted 
				...
				inner join dbo.dokvrsta dv1 on dokstart_1.dokvrsta = dv1.sifra 
			...
			from  inserted 
				...
				left outer join (select inserted.id, evidpad.analitikaid 
								from inserted 
									inner join dbo.evid on inserted.id = dbo.evid.listid
									inner join dbo.evidpad on dbo.evid.id = dbo.evidpad.evidid
									inner join dbo.analitika on dbo.evidpad.analitikaid = dbo.analitika.id
