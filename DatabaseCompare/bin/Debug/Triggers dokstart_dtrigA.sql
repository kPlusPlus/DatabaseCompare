...
begin
	...
	begin
		...
		where dbo.dokstart.id in 
			...
			from deleted
				inner join dbo.dokvk on deleted.vk = dbo.dokvk.sifra
	...
	if exists (select * from deleted
				...
				inner join dbo.evid with(nolock)on deleted.listid = dbo.evid.listid
...
where vezaid in (select id from deleted, dbo.dokvrsta 
			where deleted.dokvrsta = dbo.dokvrsta.sifra 
				...
				and (skupdogadjaj in (select dbo.dokvrsta.autoskupdog1 
							from deleted, dbo.dokvrsta 
							where deleted.dokvrsta = dbo.dokvrsta.sifra)
	or skupdogadjaj in (select dbo.dokvrsta.autoskupdog2 from deleted, dbo.dokvrsta 
				where deleted.dokvrsta = dbo.dokvrsta.sifra))
