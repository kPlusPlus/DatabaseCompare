...
begin
		...
		where dokstart.id <> dsp.id and inserted.dokumid <> dop.dokumid
...
update dbo.listzt
		set  postocarina = case when isnull(ac.postocar,0) <> 0 
					...
					else dbo.listzt.postocarina 
					end, 
			postozt = case when isnull(ac.postozav,0) <> 0 
					then  isnull(ac.postozav,0) 
					else dbo.listzt.postozt 
					end
		from dbo.listzt 
			left outer join (select  dbo.dokstart.listid as id, 
							postocar = sum(case when dbo.ztdokum.ztvrsta = 1 then
						dbo.ztdokum.iznos * dbo.ztdokum.iznposto * 0.01  dbo.listzt.iznosroba * 100  else 0 end)
					from inserted
						inner join dbo.dokstart on inserted.dokumid = dbo.dokstart.id
					...
					where dbo.dokvrsta.zt = 1 and dbo.dokvrsta.zaliha = 1 
						and dbo.listzt.iznosroba <> 0 and dbo.ztdokum.iznposto > 0
					group by dbo.dokstart.listid) ac on dbo.listzt.listid = ac.id
option (keepfixed plan)
 listzt.iznosroba * 100) as postocar
					from inserted
						inner join dbo.dokstart on inserted.dokumid = dbo.dokstart.id
						inner join dbo.dokvrsta with(nolock) on dbo.dokstart.dokvrsta = dbo.dokvrsta.sifra
						inner join dbo.ztdokum on  dbo.dokstart.listid = dbo.ztdokum.listid
					...
					group by dokstart.listid) 
				ac on dbo.listzt.listid = ac.id
			left outer join (select  dokstart.listid as id, 
						sum(ztdokum.iznos * ztdokum.iznposto * 0.01 
...
option (keepfixed plan)
	...
	begin
		...
		from  inserted 
			...
			inner join dbo.pozicija with(nolock) on dbo.dokshema.pozicija = dbo.pozicija.sifra 
			inner join dbo.analitika analitika_1 with(nolock) on inserted.analitika1id = analitika_1.id 
			inner join dbo.analitika analitika_2 with(nolock) on inserted.analitika2id = analitika_2.id 
			inner join dbo.pdvstatus with(nolock) on inserted.pdvstatus = dbo.pdvstatus.sifra 
			left outer join dbo.analitika analitika_3 with(nolock) on inserted.analitika3id = analitika_3.id
			left outer join dbo.analitpret analitpret_1 with(nolock) on analitika_1.id = analitpret_1.analitikaid 
			left outer join dbo.analitika analitpret_11 with(nolock) on analitpret_1.analitpretid = analitpret_11.id 
			left outer join dbo.analitpret analitpret_2 with(nolock) on analitika_2.id = analitpret_2.analitikaid 
			left outer join dbo.analitika analitpret_22 with(nolock) on analitpret_2.analitpretid = analitpret_22.id 
...
option (keepfixed plan)
		...
		begin
			...
			from     inserted 
				inner join dbo.dokstart on inserted.dokumid = dbo.dokstart.id
				inner join dbo.dokshema with(nolock) on dbo.dokstart.vk = dbo.dokshema.vk 
				inner join dbo.pozicija pozds with(nolock) on dbo.dokshema.pozicija = pozds.sifra 
				inner join dbo.dokshemarasp with(nolock) on dbo.dokshema.id = dbo.dokshemarasp.dokshemaid 
				left outer join dbo.analitfirma with(nolock) on dbo.dokshemarasp.analitraspid = analitfirma.id 
				left outer join dbo.analitika analitrasp with(nolock) on dbo.analitfirma.analitikaid = analitrasp.id 
				inner join dbo.pozicija with(nolock) on dbo.dokshemarasp.pozicijarasp = dbo.pozicija.sifra 
				inner join dbo.pdvstatus with(nolock) on inserted.pdvstatus = dbo.pdvstatus.sifra
				inner join dbo.analitika analitika_1 with(nolock) on inserted.analitika1id = analitika_1.id 
				inner join dbo.analitika analitika_2 with(nolock) on inserted.analitika2id = analitika_2.id 
				left outer join dbo.analitika analitika_3 with(nolock) on inserted.analitika3id = analitika_3.id
...
option (keepfixed plan)
			...
			set dokumid = inserted.dokumid, ks = case when poz in(2990,2994,2993) then 0 else 1 end
			from inserted
				inner join dbo.dokstart on inserted.dokumid = dbo.dokstart.id
				inner join dbo.dokvrsta on dbo.dokstart.dokvrsta = dbo.dokvrsta.sifra
				inner join (select inserted.dokumid, dokstavka.id, pozicija.sifra as poz
						from   inserted
					...
					dp on inserted.dokumid = dp.dokumidinner join dbo.dokstavka on dp.id  = dbo.dokstavka.id
		...
		from dbo.dokstavka 
			...
			inner join (select  dokstavka.dokumid, 
					...
					from  inserted
						...
						inner join dbo.dokstavka on inserted.dokumid = dbo.dokstavka.dokumid
						inner join dbo.pozicija with(nolock) on dbo.dokstavka.pozicija = dbo.pozicija.sifra
						inner join dbo.pozicija pzs with(nolock) on dbo.pozicija.sifra = pzs.pozsum
					 where dokvrsta.financ = 1 and dokvrsta.vrsta = 7 
...
option (keepfixed plan)
	...
	for this period'
			...
			if not exists (select * from inserted
							...
							inner join dbo.dokstavka with(nolock) on inserted.dokumid = dbo.dokstavka.dokumid
						where dokvrsta.financ = 1 and dokvrsta.vrsta = 8 and dokvrsta.sifra = 51 
							and dbo.dokstavka.pozicija = 4515 )
			begin	
				...
				from inserted
					...
					inner join dbo.analitika with(nolock) on inserted.analitika2id = dbo.analitika.id
					inner join dbo.cjelina with(nolock) on dbo.analitika.vezaid = dbo.cjelina.id		 
					inner join dbo.lokacija with(nolock) on dbo.cjelina.id = dbo.lokacija.cjelinaid
...
option (keepfixed plan)
				...
				from inserted
					...
					inner join dbo.analitika analitika_1 with(nolock) on dbo.os.id = analitika_1.vezaid
...
option (keepfixed plan)
	...
	begin
		...
		if exists(select * from inserted 
				inner join dbo.dokstart with(nolock) on inserted.dokumid = dbo.dokstart.id
				inner join dbo.dokvrsta with(nolock) on dbo.dokstart.dokvrsta = dbo.dokvrsta.sifra
				left outer join (select distinct ds.listid 
									from inserted
									inner join dbo.dokstart with(nolock) on inserted.dokumid = dbo.dokstart.id
									inner join dbo.dokstart ds with(nolock) on dbo.dokstart.listid = ds.listid 
									inner join dbo.verifikacija with(nolock) on ds.id = dbo.verifikacija.dokumid
														and dbo.verifikacija.verifvrsta = 50 
														and verifikacija.verificirano is not null) verpred on dbo.dokstart.listid = verpred.listid
			where dokvrsta.rpcp = 1 and rpcpnacin = 2 
				and dokstart.knjizeno = 0 and dokstart.zakljuceno = 0 
				and dokstart.azuriranje = 1 
				and verpred.listid is null 
				 )
		begin
...
option (keepfixed plan)
			...
			from inserted
				...
				left outer join dbo.evidpopust with(nolock) on dbo.evid.id = dbo.evidpopust.evidid
...
option (keepfixed plan)
			...
			insert into dbo.evidpopust(evidid, skalarpcp, postotak)
			(select distinct evid.id, 0, partner.dodpop  
			from inserted
				inner join dbo.dokstart with(nolock) on inserted.dokumid = dbo.dokstart.id
				inner join dbo.evid with(nolock) on dbo.dokstart.listid = dbo.evid.listid
				...
				inner join dbo.partner with(nolock) on dbo.analitika.vezaid = dbo.partner.partnerid
				left outer join dbo.dokpoprat with(nolock) on dbo.dokstart.id = dbo.dokpoprat.dokumid
				left outer join dbo.dokstart dsp with(nolock) on dbo.dokpoprat.dokpopratid = dsp.id
...
option (keepfixed plan)
			...
			insert into dbo.evidpopust(evidid, skalarpcp, postotak)
			(select distinct evid.id, skalarpcp.sifra,
				popust = (case 
				  ...
				  when pragvrsta = 31 and listnad.listvrstplac > 0 and listnad.listvrstplac = prag7 then rpcp7
				  when pragvrsta = 31 and listnad.listvrstplac > 0 and listnad.listvrstplac = prag6 then rpcp6
				  when pragvrsta = 31 and listnad.listvrstplac > 0 and listnad.listvrstplac = prag5 then rpcp5
				  when pragvrsta = 31 and listnad.listvrstplac > 0 and listnad.listvrstplac = prag4 then rpcp4
				  when pragvrsta = 31 and listnad.listvrstplac > 0 and listnad.listvrstplac = prag3 then rpcp3
				  when pragvrsta = 31 and listnad.listvrstplac > 0 and listnad.listvrstplac = prag2 then rpcp2
				  when pragvrsta = 31 and listnad.listvrstplac > 0 and listnad.listvrstplac = prag1 then rpcp1
			...
			from inserted
				...
				inner join dbo.analitika with(nolock) on inserted.analitika1id = dbo.analitika.id
							and dbo.analitika.analitikavrsta = 2
				inner join dbo.partner with(nolock) on dbo.analitika.vezaid = dbo.partner.partnerid
				inner join dbo.partnerskalarpcp with(nolock) on dbo.partner.partnerid = dbo.partnerskalarpcp.partnerid
				inner join dbo.skalarpcp with(nolock) on dbo.partnerskalarpcp.skalarpcp = dbo.skalarpcp.sifra
				...
				inner join (select inserted.dokumid,
					...
					from inserted 
						...
						inner join dbo.evid e with(nolock) on ds.listid = e.listid			
						inner join dbo.obracjedinica with(nolock) on e.obracjedinica = dbo.obracjedinica.sifra
					group by inserted.dokumid) es on inserted.dokumid = es.dokumid
				inner join dbo.list with(nolock) on dbo.dokstart.listid = dbo.list.id
				left outer join dbo.listnad with(nolock) on dbo.list.listnadid = dbo.listnad.id
				left outer join dbo.dokpoprat with(nolock) on dbo.dokstart.id = dbo.dokpoprat.dokumid
				left outer join dbo.dokstart dsp with(nolock) on dbo.dokpoprat.dokpopratid = dsp.id
						and dsp.zakljuceno = 1 
				left outer join dbo.evidpopust with(nolock) on dbo.evid.id = dbo.evidpopust.evidid 
								and dbo.skalarpcp.sifra = dbo.evidpopust.skalarpcp
			where asovrsta.pop = 1 and dokvrsta.rpcp = 1 and rpcpnacin = 2 
					and dsp.id is null and evidpopust.evidid is null)
option (keepfixed plan)

			insert into dbo.evidpopust(evidid, skalarpcp, postotak)
			(select distinct evid.id, skrpcp_ac.sifra,
				popust = case
			...
			from insertedinner join dbo.dokstart with(nolock) on inserted.dokumid = dbo.dokstart.id
				inner join dbo.evid with(nolock) on dbo.dokstart.listid = dbo.evid.listid
				inner join dbo.asokatalog with(nolock) on dbo.evid.asokatalogid = dbo.asokatalog.id
				inner join dbo.asortiman with(nolock) on dbo.asokatalog.asortid = dbo.asortiman.id
				inner join dbo.asovrsta with(nolock) on dbo.asokatalog.asovrsta = dbo.asovrsta.sifra
				inner join dbo.dokvrsta with(nolock) on dbo.dokstart.dokvrsta = dbo.dokvrsta.sifra
				inner join dbo.list with(nolock) on dbo.dokstart.listid = dbo.list.id
				inner join dbo.asocjenik with(nolock) on dbo.evid.asokatalogid = dbo.asocjenik.asokatalogid
						and dbo.list.cjenik = dbo.asocjenik.cjenik
				inner join dbo.asocjenikskalarpcp with(nolock) on dbo.asocjenik.id = dbo.asocjenikskalarpcp.asocjenikid 
				inner join dbo.skalarpcp skrpcp_ac with(nolock) on dbo.asocjenikskalarpcp.skalarpcp = skrpcp_ac.sifra
				left outer join dbo.dokpoprat with(nolock) on dbo.dokstart.id = dbo.dokpoprat.dokumid
				left outer join dbo.dokstart dsp with(nolock) on dbo.dokpoprat.dokpopratid = dsp.id
						and dsp.zakljuceno = 1 
				left outer join dbo.evidpopust with(nolock) on dbo.evid.id = dbo.evidpopust.evidid 
								and skrpcp_ac.sifra = dbo.evidpopust.skalarpcp
			where asovrsta.pop = 1 and dokvrsta.rpcp = 1 and rpcpnacin = 2 
					and dsp.id is null and evidpopust.evidid is null)
option (keepfixed plan)
			...
			from inserted
				...
				inner join dbo.listnad with(nolock) on dbo.list.listnadid = dbo.listnad.id
				inner join dbo.listvrstplac with(nolock) on dbo.listnad.listvrstplac = dbo.listvrstplac.sifra
				inner join dbo.listvrstplacskalarpcp with(nolock) on dbo.listvrstplac.sifra = dbo.listvrstplacskalarpcp.listvrstplac 
				inner join dbo.skalarpcp skrpcp_ac with(nolock) on dbo.listvrstplacskalarpcp.skalarpcp = skrpcp_ac.sifra
				left outer join dbo.dokpoprat with(nolock) on dbo.dokstart.id = dbo.dokpoprat.dokumid
				left outer join dbo.dokstart dsp with(nolock) on dbo.dokpoprat.dokpopratid = dsp.id
						and dsp.zakljuceno = 1 
				left outer join dbo.evidpopust with(nolock) on dbo.evid.id = dbo.evidpopust.evidid 
								and skrpcp_ac.sifra = dbo.evidpopust.skalarpcp						
			where asovrsta.pop = 1 and dokvrsta.rpcp = 1 and rpcpnacin = 2 
				and dsp.id is null and listnad.listvrstplac > 0 and evidpopust.evidid is null)
option (keepfixed plan)
		...
		from inserted
			...
			inner join dbo.evid on dbo.dokstart.listid = dbo.evid.listid
			inner join dbo.asocjenik with(nolock) on dbo.list.cjenik = dbo.asocjenik.cjenik
			...
			inner join (select evid.id, 
				...
				from inserted
					...
					inner join dbo.evid on dbo.dokstart.listid = dbo.evid.listid
					inner join dbo.evidpopust on dbo.evid.id = dbo.evidpopust.evidid
					left outer join dbo.skalarpcp on dbo.evidpopust.skalarpcp = dbo.skalarpcp.sifra
				where dokvrsta.rpcp = 1 and rpcpnacin = 2 
						and (skalarpcp.vrsta < 3 or skalarpcp.vrsta is null) 
...
option (keepfixed plan)
		...
		begin
			...
			from (select inserted.dokumid, dbo.obrsifra.pozicija, 
					...
					inner join dbo.dokvrsta dv with(nolock) on ds.dokvrsta = dv.sifra
			...
			set iznos = case when ukkolicina > 0 then isnull(ukkolicina,0) else 0 end,
				...
				from inserted 
					...
					left outer join (select inserted.dokumid, 
			 		...
			 		on tbpec.dokumid = seobj.dokumid and tbpec.pozicija = seobj.obracjedinica
...
option (keepfixed plan)
			...
			if exists (select * from inserted 
					 ...
					 where pozicija.ez = 1 
						...
						and pozicija.asovrstavrsta is null)
			begin
			 koef * (porez4 * 0.01) *  inserted.tecaj * kolicina2 ),2)
						+ round((fakcijena * (1- (fakpopust * 0.01))  koef * (porez2 * 0.01) *  inserted.tecaj * kolicina2 ),2)
						+ round((round((fakcijena * (1- (fakpopust * 0.01))  koef * (porez3 * 0.01) *  inserted.tecaj * kolicina2 ),2))
							* (porez1 * 0.01),2))
						end),
						osnovicatbval = sum(case when primjmp = 0 
						then round((fakcijena * (1- (fakpopust * 0.01)) * kolicina2 * 1),2)
						else round((fakcijena * (1- (fakpopust * 0.01)) * 1 * kolicina2 * 1),2)
						-(round((fakcijena * (1- (fakpopust * 0.01))  koef * (porez3 * 0.01) * 1 * kolicina2 ),2)
						+ round((fakcijena * (1- (fakpopust * 0.01))  koef * 1 * kolicina2 ),2) +
							 round((fakcijena * (1- (fakpopust * 0.01))  koef * (porez4 * 0.01) *  inserted.tecaj * kolicina2 ),2)
						+ round((fakcijena   koef * (porez2 * 0.01) *  inserted.tecaj * kolicina2),2)
						+ round(((round((fakcijena   koef * (porez3 * 0.01) *  inserted.tecaj * kolicina2),2))) * (porez1 * 0.01),2)))
						* (fakpopust * 0.01),2)
						end),
						popusttbval = sum(case when primjmp = 0 
						then round((fakcijena * (fakpopust * 0.01) *  kolicina2),2)
						else round(( round((fakcijena * kolicina2),2)
						- ( round((fakcijena   koef * (porez3 * 0.01) * kolicina2 ),2)
						+ round((fakcijena  koef  * kolicina2),2) +
						   round((fakcijena   koef * (porez4 * 0.01) *  inserted.tecaj * kolicina2 ),2)
						end),
						porez4tbval = sum(case when primjmp = 0 
						then round((fakcijena * (1- (fakpopust * 0.01)) * (porez4 * 0.01) * kolicina2 * 1),2)
						else round((fakcijena * (1- (fakpopust * 0.01))  koef * (porez3 * 0.01) *  inserted.tecaj * kolicina2 ),2)
						end),
						porez3tbval = sum(case when primjmp = 0 
						then round((fakcijena * (1- (fakpopust * 0.01)) * (porez3 * 0.01) * kolicina2 * 1),2)
						else round((fakcijena * (1- (fakpopust * 0.01))  koef * (porez2 * 0.01) *  inserted.tecaj * kolicina2),2)
						end),
						porez2tbval = sum(case when primjmp = 0 
						then round((fakcijena * (1- (fakpopust * 0.01)) * (porez2 * 0.01) * kolicina2 * 1),2)
						else round((fakcijena * (1- (fakpopust * 0.01))  koef * inserted.tecaj * kolicina2 ),2) +
							 round((fakcijena * (1- (fakpopust * 0.01))  koef * 1 * kolicina2 ),2) +
							 round((fakcijena * (1- (fakpopust * 0.01))  inserted.tecaj)
								else (dbo.zaliha.trosarina * dbo.skladiste.kolicina  inserted.tecaj)
								else (dbo.zaliha.prcijenabez * dbo.skladiste.kolicina  inserted.tecaj)
								else (dbo.zaliha.cijena * dbo.zaliha.zavisni * 0.01 * dbo.skladiste.kolicina  inserted.tecaj)
								else (dbo.zaliha.cijena * dbo.zaliha.carina * 0.01 * dbo.skladiste.kolicina  inserted.tecaj)
								else (dbo.zaliha.cijena * dbo.skladiste.kolicina  inserted.tecaj)
								else (dbo.zaliha.markup * dbo.skladiste.kolicina  inserted.tecaj)
								else (dbo.zaliha.prcijenabez  * dbo.portarif.porez4 * 0.01 * dbo.skladiste.kolicina  inserted.tecaj)
								else (dbo.zaliha.prcijenabez  * dbo.portarif.porez3 * 0.01 * dbo.skladiste.kolicina  inserted.tecaj)
								else (dbo.zaliha.prcijenabez  * dbo.portarif.porez2 * 0.01 * dbo.skladiste.kolicina  inserted.tecaj
			...
			update tbpec  
				...
				when pozicija = 1850 then  porez3tb
				when pozicija = 1900 then  porez4tb
				when pozicija = 1950 then  trosarinatb
				when pozicija = 1961 then takstrost1
				when pozicija = 1962 then takstrost2
				...
				when pozicija = 2800 then tbpec.iznos 
				...
				when pozicija = 1961 then takstrost1val
				when pozicija = 1962 then takstrost2val
				...
				when pozicija = 2800 then tbpec.iznosval 
			...
			from inserted 
				...
				inner join (select inserted.dokumid,
					...
					porez1tb3 = sum(case when ze.portarifid = 9 then porez1tb else 0end), 
					...
					neopor = sum(case when ze.portarifid = 0 then osnovtb else 0 end), 
					osnovtb0 = sum(case when ze.portarifid = 6 then osnovtb else 0 end), 
					osnovtb1 = sum(case when ze.portarifid = 7 then osnovtb else 0 end), 
					osnovtb2 = sum(case when ze.portarifid = 8 then osnovtb else 0 end), 
					osnovtb3 = sum(case when ze.portarifid = 9 then osnovtb else 0 end), 
					osnovtb4 = sum(case when ze.portarifid = 10 then osnovtb else 0 end), 
					neoporval = sum(case when ze.portarifid = 0 then osnovtbval else 0 end), 
					osnovtb0val = sum(case when ze.portarifid = 6 then osnovtbval else 0 end), 
					osnovtb1val = sum(case when ze.portarifid = 7 then osnovtbval else 0 end), 
					osnovtb2val = sum(case when ze.portarifid = 8 then osnovtbval else 0 end), 
					osnovtb3val = sum(case when ze.portarifid = 9 then osnovtbval else 0 end), 
					osnovtb4val = sum(case when ze.portarifid = 10 then osnovtbval else 0 end),
					takstrost1 = sum (takstrost1), 
					takstrost2 = sum (takstrost2),
					takstrost1val = sum (takstrost1val), 
					takstrost2val = sum (takstrost2val)
				from inserted 
					inner join (select inserted.dokumid,  dbo.portarif.id as portarifid,
						...
						else round(((fakcijena * (1- (fakpopust * 0.01))+ takstros.jediznt2 + takstros.jedizn) * kolicina2 *  inserted.tecaj),2)
						...
						+ round((round(((fakcijena * (1- (fakpopust * 0.01))+ takstros.jediznt2) * kolicina2 * inserted.tecaj),2) + 
							round((fakcijena * (1- (fakpopust * 0.01)) * (porez3 * 0.01) * kolicina2 *  inserted.tecaj),2))
							* (porez1 * 0.01),2)
						end),
						iznostbval = sum(case when primjmp = 1 
						then round((fakcijena * (1- (fakpopust * 0.01)) * kolicina2 * 1),2)
						else round(((fakcijena * (1- (fakpopust * 0.01)) + takstros.jediznt2 + takstros.jedizn) * kolicina2 * 1),2)
						...
						+ round((round(((fakcijena * (1- (fakpopust * 0.01))+ takstros.jediznt2) * kolicina2 ),2) + 
							round((fakcijena * (1- (fakpopust * 0.01)) * (porez3 * 0.01) * kolicina2 ),2))
							* (porez1 * 0.01),2)
						end),
						osnovicatb = sum(case when primjmp = 0 
						then round((fakcijena * (1- (fakpopust * 0.01)) * kolicina2 *  inserted.tecaj),2)
						else  round(((fakcijena * (1- (fakpopust * 0.01))-takstros.jedizn) * kolicina2 *  inserted.tecaj),2)
						...
						+ round((round(((fakcijena * (1- (fakpopust * 0.01))-takstros.jedizn)  koef * (porez3 * 0.01) * 1 * kolicina2),2))
							* (porez1 * 0.01),2))
						end),
						osnovtb =   sum(case when primjmp = 0 
						then round(((fakcijena * (1- (fakpopust * 0.01))+ takstros.jediznt2) * kolicina2 *  inserted.tecaj),2)

						else  round(((fakcijena * (1- (fakpopust * 0.01))-takstros.jedizn) * kolicina2 *  inserted.tecaj),2)
						- (round((fakcijena * (1- (fakpopust * 0.01))  koef * (porez3 * 0.01) *  inserted.tecaj * kolicina2 ),2)
						+ round((fakcijena * (1- (fakpopust * 0.01))  koef *  inserted.tecaj * kolicina2 ),2) +
							 round((fakcijena * (1- (fakpopust * 0.01))  koef * (porez4 * 0.01)  * kolicina2 ),2)
						+ round((fakcijena * (1- (fakpopust * 0.01))  koef * (porez2 * 0.01)  * kolicina2 ),2)
						+ round((round(((fakcijena * (1- (fakpopust * 0.01))-takstros.jedizn)  koef * (porez3 * 0.01)  * kolicina2),2))
							* (porez1 * 0.01),2))
						end),
						popusttb = sum(case when primjmp = 0 
						then round((fakcijena * (fakpopust * 0.01) *  kolicina2 *  inserted.tecaj),2)
						else round(( round(((fakcijena - takstros.jedizn) * kolicina2 *  inserted.tecaj),2)
						...
						+ round(((round(((fakcijena - takstros.jedizn)  koef * (porez3 * 0.01) * kolicina2),2))) * (porez1 * 0.01),2)))
						* (fakpopust * 0.01),2)
						end),
						...
						then round((round(((fakcijena * (1- (fakpopust * 0.01))+ takstros.jediznt2) * kolicina2 *  inserted.tecaj),2) + 
							round((fakcijena * (1- (fakpopust * 0.01)) * (porez3 * 0.01) * kolicina2 *  inserted.tecaj),2))
							* (porez1 * 0.01),2)
						else round((round(((fakcijena * (1- (fakpopust * 0.01))-takstros.jedizn)  koef * (porez3 * 0.01) *  inserted.tecaj * kolicina2 ),2))
							* (porez1 * 0.01),2)
						end),
						porez1tbval = sum(case when primjmp = 0 
						then round((round(((fakcijena * (1- (fakpopust * 0.01))+ takstros.jediznt2) * kolicina2 * 1),2) + 
							round((fakcijena * (1- (fakpopust * 0.01)) * (porez3 * 0.01) * kolicina2 * 1),2))
							* (porez1 * 0.01),2)
						else round((round(((fakcijena * (1- (fakpopust * 0.01))-takstros.jedizn)  koef * (porez3 * 0.01) * 1 * kolicina2),2))
							* (porez1 * 0.01),2)
						end),
						...
						round(sum(kolicina2 * takstros.jedizn * inserted.tecaj),2) as takstrost1,
						round(sum(kolicina2 * takstros.jedizn),2) as takstrost1val,
						round(sum(kolicina2 * takstros.jediznt2 * inserted.tecaj),2) as takstrost2,
						round(sum(kolicina2 * takstros.jediznt2),2) as takstrost2val
					from    inserted
						inner join dbo.dokstart on inserted.dokumid = dbo.dokstart.id
						inner join dbo.list on dbo.dokstart.listid = dbo.list.id 
						...
						inner join dbo.portarif with(nolock) on dbo.evid.portarifid = dbo.portarif.id
						inner join dbo.takstros with(nolock) on dbo.evid.takstros = dbo.takstros.sifra
				...
				inner join (select inserted.dokumid,
					...
					sum(trosarinaiznos) astrosarinaiznos, sum(trosarinaiznosval) as trosarinaiznosval, 
				...
				from inserted 
					left outer join (select inserted.dokumid,  dbo.portarif.id as portarifid,
						...
						iznpor1 = round(sum(isnull(case when dbo.zaliha.preractecaj = 0 
								...
								else (dbo.zaliha.prcijenabez  * (1+(dbo.portarif.porez3 * 0.01)))* dbo.portarif.porez1 * 0.01 * dbo.skladiste.kolicina
								end,0)),2),
					...
					from    inserted
						...
						inner join dbo.portarif with(nolock) on dbo.zaliha.portarifid = dbo.portarif.id
					group by  inserted.dokumid, dbo.portarif.id) 
...
option (keepfixed plan)
			...
			if exists (select * from inserted 
						inner join dbo.dokstavka with(nolock) on inserted.dokumid = dbo.dokstavka.dokumid
						inner join dbo.pozicija with(nolock) on dbo.dokstavka.pozicija = dbo.pozicija.sifra
					 where pozicija.ez = 1 and pozicija.asovrstavrsta > 0)
			begin
			update 	tbpec  
				set iznos = isnull(case 
				...
				when (pozicija between 4152 and 4180) then  dobavaiznosval + zavisniiznosval + carinaiznosval
				else 0 
				end,0)
			from inserted 

				inner join @tbpec tbpec on inserted.dokumid = tbpec.dokumid
				inner join dbo.pozicija with(nolock) on tbpec.pozicija = dbo.pozicija.sifra 
				...
				left outer join (select inserted.dokumid, dbo.asovrsta.vrsta,
					...
					from    inserted
						...
						inner join  dbo.asokatalog with(nolock) on evidzad.asokatalogid = dbo.asokatalog.id
						inner join dbo.asovrsta with(nolock) on dbo.asokatalog.asovrsta = dbo.asovrsta.sifra
					group by  inserted.dokumid, dbo.asovrsta.vrsta) 
...
option (keepfixed plan)
		...
		declare 	@fiznos money,
			...
			select @fiznos = isnull((select round(sum(iznoskamate),2) 
						from inserted 
							inner join dbo.kamateobstavka on inserted.dokumid = kamateobstavka.dokumid
						group by inserted.dokumid),0) 
option (keepfixed plan)		

			select @fosnpdv =round(isnull((select osnovpdv  = sum(case 
...
option (keepfixed plan)
			...
			select @fpdv =round(	isnull((select     pdv =sum(case  
					when intkamata>kamata and intkoristi = 1
					then cast(dbo.kamateobstavka.osnovica as float) * 
					cast((dbo.kamata.intkamata-dbo.kamata.kamata) as float) * 
				...
				from dbo.kamateobstavka 
					inner join dbo.kamata on dbo.kamateobstavka.kamataid = dbo.kamata.id 
					inner join dbo.portarif with(nolock) on portarif.id=kamata.portarifid
				where	(dbo.kamata.intkoristi = 1)
				group by dbo.kamateobstavka.dokumid
				having      (dbo.kamateobstavka.dokumid = (select dokumid from inserted))),0),2)
option (keepfixed plan)
						...
						from dbo.dokstavka 
							inner join (select inserted.dokumid, 
									...
									from inserted 
										...
										inner join dbo.kamateobstavka on ugovororig.dokumid = dbo.kamateobstavka.dokumid
										inner join dbo.kamata with(nolock) on dbo.kamateobstavka.kamataid = dbo.kamata.id 
										left outer join dbo.portarif with(nolock) on dbo.kamata.portarifid = dbo.portarif.id
									where     (dateadd(day, dbo.kamateobstavka.period, ugovororig.datum) =inserted.datum)
						...
						from dbo.dokstavka 
							inner join (select inserted.dokumid, round(dbo.kamateobstavka.iznosanuitet,2) as iznosanuitet, 
								...
								from inserted 
									inner join dbo.dokstart on inserted.dokumid = dbo.dokstart.id 
									inner join dbo.list on dbo.dokstart.listid = dbo.list.id 
									inner join dbo.listnad with(nolock) on dbo.list.listnad1id = dbo.listnad.id and dbo.listnad.skupdogadjaj=7
