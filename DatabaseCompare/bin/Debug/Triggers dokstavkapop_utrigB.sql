...
begin
	...
	if exists (select * from  dbo.firma where naziv like 'avon%' )
	begin
	...
	begin
		...
		select dbo.dokstavka.dokumid, 
			pozicija = case when dokstavka.pozicija = 2993 
							and dokvrsta.mp = 1 
						then 2991 
						else dbo.dokstavka.pozicija
						end,
			...
			dbo.dokstavka.pozdatum, 
			dp = case when dokstavka.pozicija = 2993 
							and dokvrsta.mp = 1 and dbo.dokstavka.dp = 1 
				then 0
				else dbo.dokstavka.dp
				end, 
			dbo.dokstavka.ps, 
		...
		from inserted 
			...
			inner join (select dbo.dokstavka.id as dokstavkaid, 
						rest = case when max(dokum.tecaj) = 1 
							then isnull(max(abs(dbo.dokstavka.iznos)) 
								- sum (case when dokstavka_2.dp = dbo.dokstavka.dp 
										then abs(dokstavka_2.iznos) 
										else dokstavka_2.iznos 
										end),max(dbo.dokstavka.iznos))
							else isnull(max(abs(dbo.dokstavka.iznosval)) 
								- sum (case when dokstavka_2.dp = dbo.dokstavka.dp 
										then abs(dokstavka_2.iznosval) 
										else dokstavka_2.iznosval
										end),max(dbo.dokstavka.iznosval))
							end
					from dbo.dokum
		...
		where dokstavka_1.protuid > 0 and pozicija.zatvaranje = 1 
			and dbo.dokstavka.dp = dokstavka_1.dp 
		...
		if exists (select * from inserted
			...
			wheredbo.dokstavka.id = dsplac.id 
