...
BEGIN TRANSACTION 
	...
	declare @siStatusPaste smallint	
...
SELECT  @siStatusPaste = isnull(dvs1.Sifra, 0) from dbo.DokStart ds with(nolock) 
					inner join dbo.DokVrsta dv with(nolock) on ds.DokVrsta = dv.Sifra
					inner join dbo.DokVrstaStatus dvs with(nolock) on ds.status = dvs.Sifra 
					inner join dbo.DokVrstaStatus dvs1 with(nolock) on dvs.Tekst = dvs1.Tekst 
					inner join dbo.DokVrsta dv1 with(nolock) on dvs1.DokVrsta = dv1.Sifra
					where ds.ID = @inDokumID And dv.Vrsta = 101 
								and dvs1.DokVrsta = @tiDokVrstaPaste 
								and dv1.Vrsta = 101 
	...
	BEGIN
		...
		INSERT INTO dbo.DokStart
			(ListID, DokVrsta, VK, Godina, Broj, Izvornik, HostID, FirmaID, Status)
			VALUES (@inNoviListID,@tiDokVrstaPaste, @siVK, @siGodina, @ncNoviBroj, @ncIzvornik, @inHostID, @inFirmaID, @siStatusPaste)
