...
option (keepfixed plan)
	...
	begin
			...
			set 	opis = case when kz.skopis = '' or kz.skopis is null
				...
				analitdodid = case when kz.analitikaid > 0 and kz.analitvrstadod > 0 
							and dokstavkaknjiga.analitdodid is null 
						then kz.analitikaid
...
option (keepfixed plan)
	...
	if exists (select * from inserted 
				...
				where inserted.azuriranje = 1 and inserted.kontrola = 1 
					and (dokvrsta.autoskupdog1 > 0 or dokvrsta.autoskupdog2 > 0) )
	begin
...
option (keepfixed plan)
			...
			update dbo.list
			set listnadid = case when ln1.id > 0 then ln1.id else dbo.list.listnadid end,
				listnad1id =  case when ln2.id > 0 then ln2.id else dbo.list.listnad1id end
			from inserted
				inner join dbo.list on inserted.listid = dbo.list.id 
				inner join dbo.dokvrsta with(nolock) on inserted.dokvrsta = dbo.dokvrsta.sifra 
				left outer join dbo.listnad ln1 with(nolock) on inserted.id = ln1.vezaid and dbo.dokvrsta.autoskupdog1 = ln1.skupdogadjaj
				left outer join dbo.listnad ln2 with(nolock) on inserted.id = ln2.vezaid and dbo.dokvrsta.autoskupdog2 = ln2.skupdogadjaj
...
option (keepfixed plan)
	...
	begin
		...
		begin
			if exists(select * from inserted 
				inner join dbo.dokstart with(nolock) on inserted.godina = dbo.dokstart.godina
					and rtrim(inserted.izvornik) = rtrim(dbo.dokstart.izvornik)
					and inserted.id <> dbo.dokstart.id
				inner join dbo.dokvrsta with(nolock) on inserted.dokvrsta = dbo.dokvrsta.sifra 
				inner join dbo.dokvrsta dvd with(nolock) on dbo.dokstart.dokvrsta = dvd.sifra 
				inner join dbo.dokum with(nolock) on dbo.dokstart.id = dbo.dokum.dokumid
				inner join dbo.dokum insdokum with(nolock) on inserted.id = insdokum.dokumid
						and dbo.dokum.analitika1id = insdokum.analitika1id
				inner join dbo.analitika with(nolock) on insdokum.analitika1id = dbo.analitika.id
							and dbo.analitika.analitikavrsta = 2
				inner join dbo.dokstavka with(nolock) on insdokum.dokumid = dbo.dokstavka.dokumid
				inner join dbo.pozicija with(nolock) on dbo.dokstavka.pozicija = dbo.pozicija.sifra
