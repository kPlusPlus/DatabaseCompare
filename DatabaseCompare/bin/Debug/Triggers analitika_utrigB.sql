...
if exists (select * from inserted where inserted.analitikavrsta = 41 and vezaid not in (select id from dbo.prikljucak))
		begin
		raiserror 44447 '
analitika_itrig: 
the record can''t be added or changed. 
referential integrity rules require a related 
record in table ''prikljucak''.
origin:analitika_utrig'
		rollback transaction
    		end
...
if exists(select * from inserted where inserted.analitikavrsta = 4)
begin
	insert into [dbo].[asosklad] ([asortid],[analitsklid],[staskl],[pondmarza], sco)
	select asortiman.id, inserted.id, 
		...
		sklpj.oscm
	from inserted
		inner join dbo.sklpj on dbo.sklpj.cjelinaid = inserted.vezaid
		...
		left outer join (select zaliha.asortid, zaliha.id as zalihaid, zaliha.analitsklid, 
						...
						from inserted
							inner join dbo.zaliha on inserted.id = dbo.zaliha.analitsklid 
	...
	group by inserted.id, asortiman.id, sklpj.oscm
end