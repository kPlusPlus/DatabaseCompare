	...
	begin
		...
		select inserted.listid,  asokatalog_1.id, 101, jmj.sifra, 
			...
			fakcijena = case when asocjenik.cijenaplkost > 0 and dokvrsta.cijenavrsta = 2 
					...
					else cast(((inserted.kolicina2 * (inserted.fakcijena - (inserted.fakcijena * inserted.fakpopust *0.01)))
						 inserted.kolicina2 as money)
					end, 

			0, 0, 
			bod = case when asocjenik.bod > 0 and dokvrsta.cijenavrsta between 2 and 3
					then asocjenik.bod
					else cast(((inserted.kolicina2 * cast(inserted.bod as money))
						 inserted.kolicina2  as smallint)
					end, 
		...
		from	inserted
			...
			inner join dbo.asonorm with(nolock) on inserted.asokatalogid = dbo.asonorm.katalogid
			inner join dbo.asokatalog asokatalog_1 with(nolock) on dbo.asonorm.komponentid = asokatalog_1.id
			inner join dbo.asortiman with(nolock) on asokatalog_1.asortid = dbo.asortiman.id
			inner join dbo.obracjedinica jmj with(nolock) on dbo.asortiman.obracjedgrupa = jmj.obrjedgrupa
...
option (keepfixed plan)
		...
		where id in (select inserted.id 
			from inserted
				...
				inner join dbo.asonorm with(nolock) on dbo.asokatalog.id = dbo.asonorm.katalogid
			where dbo.dokvrsta.setovi = 0 and dokvrsta.normativ = 0 
				and (inserted.tip < 99 or inserted.tip > 999) 
...
option (keepfixed plan)
	...
	begin
		...
		from  inserted
			...
			inner join dbo.asokatalog with(nolock) on inserted.asokatalogid = dbo.asokatalog.id
			inner join dbo.asoutrosak with(nolock) on dbo.asokatalog.asortid = dbo.asoutrosak.asortid
			inner join dbo.asortiman with(nolock) on dbo.asoutrosak.asokompid = dbo.asortiman.id
			inner join dbo.obracjedinica with(nolock) on inserted.obracjedinica = dbo.obracjedinica.sifra
			inner join dbo.obracjedinica asortjmj with(nolock) on dbo.asoutrosak.asortjmj = asortjmj.sifra
			inner join dbo.obracjedinica asokompjmj with(nolock) on dbo.asoutrosak.asokompjmj = asokompjmj.sifra
...
option (keepfixed plan)
		...
		begin
			...
			from inserted
				inner join dbo.dokstart on inserted.listid = dbo.dokstart.listidinner join dbo.dokvrsta with(nolock) on dbo.dokstart.dokvrsta = dbo.dokvrsta.sifra
