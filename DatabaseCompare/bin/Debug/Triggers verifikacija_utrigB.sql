...
begin
	...
	if exists (	select * 
			...
			where  checksum(d.zatrazio, d.zatrazeno, d.verifvrsta, d.dokumid) 
				<> checksum(i.zatrazio, i.zatrazeno, i.verifvrsta, i.dokumid) 
	...
	begin
		...
		from inserted
			...
			left outer join dbo.dokstart ulka on dbo.dokstart.listid = ulka.listid 
								and dbo.dokstart.id < ulka.id
		where (ulka.knjizeno = 1 or ulka.zakljuceno = 1 
			or (dokvrsta.sklad = 1 and dokvrsta.zadrazd = 1)) 
