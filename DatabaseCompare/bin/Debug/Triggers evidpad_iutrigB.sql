...
begin
	...
	begin
		...
		if exists (select * from inserted
				...
				where len(isnull(asokatalog.barcode, n'')) < 3)
		begin
...
option (keepfixed plan)
			...
			from inserted
				inner join dbo.evid on inserted.evidid = dbo.evid.id
				inner join dbo.asokatalog on dbo.evid.asokatalogid = dbo.asokatalog.idinner join dbo.dokstart on dbo.evid.listid = dbo.dokstart.listid
				inner join dbo.dokvrsta on dbo.dokstart.dokvrsta = dbo.dokvrsta.sifra 
...
option (keepfixed plan)
		...
		begin
			...
			insert into [dbo].[zalserbr] (zalihaid, serija, broj, napomena)
			select zaliha.id, null, rtrim(inserted.barcodebach), null
			from inserted 
...
option (keepfixed plan)
			...
			from inserted 
				...
				inner join ( 
						...
						from dbo.zaliha zinner join dbo.zalserbr zsb on zsb.zalihaid = zsb.zalihaid
